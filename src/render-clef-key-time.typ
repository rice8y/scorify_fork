...
#import "glyph-metadata.typ": place-glyph, advance-width, make-music-font-config

// ===== Width calculation functions (pure, no drawing) =====

/// Compute the width of a clef in staff-space units.
#let clef-advance(clef-name: "treble", sp: 1.0, font-config: make-music-font-config()) = {
  let smufl = clef-smufl-name.at(clef-name, default: "gClef")
  advance-width(smufl, font-config: font-config) * sp + default-clef-padding * sp
}

/// Compute the width of a key signature in staff-space units.
#let key-sig-advance(key-name, sp: 1.0, font-config: make-music-font-config()) = {
  ...
  if n == 0 { 0.0 }
    ...
    let acc-w = advance-width(acc-smufl, font-config: font-config)
    n * (acc-w + 0.2) * sp + default-key-sig-padding * sp
  }
}

/// Compute the width of a time signature in staff-space units.
#let time-sig-advance(upper, lower, symbol: none, sp: 1.0, font-config: make-music-font-config()) = {
  if symbol == "common" or symbol == "cut" {
    let name = if symbol == "common" { "timeSigCommon" } else { "timeSigCutCommon" }
    advance-width(name, font-config: font-config) * sp + default-time-sig-padding * sp
  } else {
    ...
    for ch in upper-str {
      if ch >= "0" and ch <= "9" {
        upper-w += advance-width("timeSig" + ch, font-config: font-config) * sp
      }
    }
    let lower-w = 0.0
    for ch in lower-str {
      if ch >= "0" and ch <= "9" {
        lower-w += advance-width("timeSig" + ch, font-config: font-config) * sp
      }
    }
...
#let draw-clef(x, y-top, clef-name, sp: 1.0, font-config: make-music-font-config()) = {
  let smufl = clef-smufl-name.at(clef-name, default: "gClef")
  ...
  place-glyph(x, origin-y, glyph, smufl, sp, font-config: font-config)
}

/// Draw a key signature at the given position (no return value).
#let draw-key-signature(x, y-top, key-name, clef-name, sp: 1.0, font-config: make-music-font-config()) = {
  let count = key-sig-accidental-count.at(key-name, default: 0)
  if count == 0 { return }
  ...
  let acc-spacing = (advance-width(acc-smufl, font-config: font-config) + 0.2) * sp
  for i in range(n) {
    let staff-pos = positions.at(i)
    let acc-y = y-top - staff-pos * sp / 2.0
    let acc-x = x + i * acc-spacing
    place-glyph(acc-x, acc-y, acc-glyph, acc-smufl, sp, font-config: font-config)
  }
}

/// Draw a time signature at the given position (no return value).
#let draw-time-signature(x, y-top, upper, lower, symbol: none, sp: 1.0, font-config: make-music-font-config()) = {
  if symbol == "common" {
    place-glyph(x, y-top - 2.0 * sp, smufl-time-common, "timeSigCommon", sp, font-config: font-config)
    return
  }

  if symbol == "cut" {
    place-glyph(x, y-top - 2.0 * sp, smufl-time-cut, "timeSigCutCommon", sp, font-config: font-config)
    return
  }
  ...
  for ch in upper-str {
    if ch >= "0" and ch <= "9" {
      ...
      place-glyph(x + dx, upper-y, glyph-char, glyph-name, sp, font-config: font-config)
      dx += advance-width(glyph-name, font-config: font-config) * sp
    }
  }
  ...
  for ch in lower-str {
    if ch >= "0" and ch <= "9" {
      let glyph-name = "timeSig" + ch
      let glyph-char = smufl-time-digits.at(int(ch))
      place-glyph(x + dx, lower-y, glyph-char, glyph-name, sp, font-config: font-config)
      dx += advance-width(glyph-name, font-config: font-config) * sp
    }
  }
}
